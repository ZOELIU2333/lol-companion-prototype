  ...
  return {
    ...
    async readSession() {
      ...
      const [phase, gameflowSession, currentSummoner] = await Promise.all([
        ...
        request<{ gameData?: { queue?: { description?: string; gameMode?: string } } }>('/lol-gameflow/v1/session'),
        request<{ displayName?: string; gameName?: string }>('/lol-summoner/v1/current-summoner'),
      ])

      if (!phase) return null

      const queue = gameflowSession?.gameData?.queue
      const mode = mapLcuQueueToMode(queue?.description ?? queue?.gameMode)
      const players = phase === 'ChampSelect' ? await readChampSelectPlayers(request) : []
