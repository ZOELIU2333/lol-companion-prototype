...
读取 2999 接口时，应用会复用连接、等待最多 2.5 秒并快速重试一次。短暂失败后的十秒内会显示“Live Client 正在重连”，保留最近一次快照并标注数据年龄；超过十秒仍未成功才会清除快照并显示不可用。

诊断显示“Live Client 正在重连”时：

1. 确认已经进入对局，而不是仅停留在选人界面。
2. 等待 3 秒或点击“重新检测”。
3. 检查安全软件是否阻止本机 2999 端口。

LCU 与 Live Client 独立判断：若 2999 有新鲜数据但 LCU 暂时掉线，顶部会显示“实时对局 · LCU 待恢复”，不会错误回到 Demo 模式。
